...
void Entity::init() {
    components = 0;
    #define _SINGLE(name, type) name = {};
    #define _MULTIPLE(name, type, amt) for (uint32_t n = 0; n < amt; ++n) { name[n] = {}; }
    PERFIELD
    #undef _SINGLE
    #undef _MULTIPLE
...
#ifdef SERVER_SIDE
#define _SINGLE(name, type) void Entity::set_##name(type const v) { if (name == v) return; name = v; state_##name = 1; }
#define _MULTIPLE(name, type, amt) void Entity::set_##name(uint32_t i, type const v) { if (name[i] == v) return; name[i] = v; state_##name = 1; state_per_##name[i] = 1; }
PERFIELD
#undef _SINGLE
#undef _MULTIPLE
...
void Entity::read(Reader *reader) {
    ...
    #define _SINGLE(name, type) case k##name: { reader->read_##type(name); state_##name = 1; break; }
